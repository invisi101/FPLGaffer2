"""MILP solver for FPL transfer optimization.

Ported from v1's solver.py — solve_transfer_milp() and
...
def solve_transfer_milp(
    ...
    # Exactly 11 starters
    add_constraint(
        np.concatenate([zeros, np.ones(n)]),
    ...
    add_constraint(np.concatenate([is_current, zeros]), keep_min, solver_cfg.squad_size)
    ...
    current_ids_in_pool = set(
        df.loc[is_current.astype(bool), "player_id"].tolist()
    )
...
def solve_transfer_milp_with_hits(
    ...
    team_cap: int = solver_cfg.team_cap,
    captain_col: str | None = None,
    hit_cost: float = solver_cfg.hit_cost,
) -> dict | None:
    ...
    required = ["player_id", "position", "cost", target_col]
    clean_pool = player_df.dropna(
        subset=[c for c in required if c in player_df.columns]
    )
    ...
    for n_transfers in range(1, max_transfers + 1):
        result = solve_transfer_milp(
            player_df, current_player_ids, target_col,
            budget=budget, max_transfers=n_transfers,
            team_cap=team_cap, captain_col=captain_col,
        )
        if result is None:
            continue
